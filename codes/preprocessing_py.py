...
cards = ['DATE-OBS', 'NAXIS1', 'NAXIS2',
         'DATA-TYP', 'EXPTIME', 'AIRMASS',
         'SLT-WID','DISPERSR','OBJECT']
...
biastab = summarytab[((summarytab['OBJECT'] == 'BIAS')
                     & (summarytab['EXPTIME'] == 0 )) ]

darktab = summarytab[((summarytab['OBJECT'] == 'BIAS')
                     & (summarytab['EXPTIME'] > 100 )) ]

flattab = summarytab[((summarytab['OBJECT'] == 'flat')
                     & (summarytab['DISPERSR'] == OBJ_GRATING)
                     & (summarytab['SLT-WID'] == OBJ_SLITWID))]
